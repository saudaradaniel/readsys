...
$result = $conn->query("SELECT `nomor_urut`, `Kodasi`, `tahun`, (`Opening` + `DOpening` + `Debit01`+ `Debit02` + `Debit03` + `Debit04` + `Debit05` + `Debit06` + `Debit07` + `Debit08`+ `Debit09` + `Debit10` + `Debit11` +  `Debit12`) AS debet, 
(`COpening` + `Credit01` +  `Credit02` + `Credit03`+`Credit04` +`Credit05` + `Credit06` + `Credit07` + `Credit08` + `Credit09` + `Credit10` + `Credit11` + `Credit12`) AS kredit,`Keterangan` FROM `readsysgeneralledger` $queryConstrain ");

$outp = "";
while($rs = $result->fetch_array(MYSQLI_ASSOC)) {
	if ($outp != "") {$outp .= ",";}
	$outp .= '{"Account":"'.  $rs["Keterangan"]. '",';
	$outp .= '"Kodasi":"'.     $rs["Kodasi"]. '",';
	$outp .= '"Debit":"'.     $rs["debet"]. '",';
